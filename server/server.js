...
app.get('/podcasts', async (req, res) => {
    const getPodLinks = await database.getAllPods()
        .then(response => {
            if (response.status) {
...
app.get('/podcast/:id', async (req, res) => {
    const getSinglePod = await database.getSinglePod(req.params.id)
        .then(response => {
            if (response.status) {
...
app.post('/save', async (req, res) => {
    const savePod = await database.saveNewPod(req.body)
        .then(response => {
            if (response.status) {
                res.send(response.doc).status(200);
            } else res.status(500);
...
app.patch('/update', async (req, res) => {
    const updatePod = await database.updatePod(req.body)
        .then(response => {
            if (response.status) {
...
app.delete('/delete', async (req, res) => {
    const deletePod = await database.deletePod(req.body.id)
        .then(response => {
            if (response.status) {
