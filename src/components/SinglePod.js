...
const SinglePod = ({ match }) => {
    return (<DetailDisplay detailedData={match.location.props.detailedData} handleDelete={match.location.props.handleDelete}/>)
}
...
const DetailDisplay = ({ detailedData, handleDelete }) => {
    ...
    return (
      <div>
        ...
        <div className="columns">
            <div className="column mt-3 search">
              <div className="control">
                <input className="input" type="text" id="searchInput" placeholder="Search for an episode!" onChange={e => handleSearch(e.target.value)}/>
              </div>
            </div>
        </div>
        <div className="columns">
            <div className="column is-one-third pod-image-container">
                <img className="pod-Img px-5 pb-3" src={detailedData.image.url} alt="pod icon"></img>
            </div>
            <div className="column is-one-third list-container">
                <ul>
                    <PodFeed detailedData={detailedData} search={search} select={handleDisplay}/>
                </ul>
            </div>
        ...
        <div className="del-button">
           <button className="button is-danger" onClick={handleButtonClick}>Delete</button>
        </div>
      </div>
    )
  }
  ...
  const PodFeed = ({ detailedData, search, select }) => {
    return (
      detailedData.items.filter(item => item.title.toLowerCase().includes(search)).map(item => {
        return (
          <li className="mt-2 mb-2" key={item.guid}>
            <div className="columns">
                <div className="column">
                    <button className="button episode" onClick={() => select(item)}>{item.title}</button>
                </div>
            </div>
          </li>
        )
