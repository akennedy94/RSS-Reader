...
const DetailDisplay = ({ match, handleDelete }) => {
    ...
    const getAndSetPod = async () => {
        const podFeed = await axios.get(`/podcastFeed/${endpoint}`)
        .then(response => {
    ...
    return (
    <React.Fragment>
      { detailedData === null ? <LoadingCard error={error} handleButtonClick={handleButtonClick}/> :
        <div>
            ...
            <div className="loading-section">
                <div className="card">
                    <div className="title is-4">
                        <p>Oh no! It looks like something went wrong loading this feed!</p>
                    </div>
                    <div className="card-content">
                        <p>This typically happens if the submitted link isn't actually and RSS feed, 
                            double check the link and try again!</p>
                    </div>
                </div>
            </div>
            <div className="back-button">
